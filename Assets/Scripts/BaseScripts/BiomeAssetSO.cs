...
public class BiomeAssetSO : ScriptableObject {
    ...
    public BiomeElement[] biomesSO;
    public BiomeSO GetBiomeSO2(float temp, float humidity) {
        for (int i = 0; i < biomesSO.Length; i++) {
            if (biomesSO[i]._maxTemperature >= temp && biomesSO[i]._minTemperature <= temp) {
                if (biomesSO[i]._maxHumidity >= humidity && biomesSO[i]._minHumidity <= humidity) {

                    return biomesSO[i]._biomeSO;
                }
            }
        }
        return ForestSO;
